...
mcmc_mh <- function(iters, burnin, init_params, tuners, y, X, Z, show_plot = TRUE) {
  ...
  log_post_prop <- log_post <- log_posterior(init_params, y, b_vals[1, ], X, Z)
  ...
  cat(sprintf("Total b acceptance:     %.3f%%\n", mean(acceptance_b)*100))
  cat(sprintf("Burned-in b acceptance: %.3f%%\n", mean(acceptance_b[-(1:burnin)])*100))
  
  list(theta = theta_vals, b = b_vals, q=mean(acceptance)*100)
}
...
q3=sample(seq(from=0.05, to=1, by=0.05),50, replace=TRUE)
q4=sample(seq(from=0.05, to=0.5, by=0.05),50, replace=TRUE)
q5=sample(seq(from=0.5, to=3, by=0.05),50, replace=TRUE)

tuning_params = rbind(q1, q2, q3, q4, q5)
#Random tuning params^

acc = 0
tuners=0
for (i in 1:dim(tuning_params)[2]){
  zz <- mcmc_mh(3000, 1000, c(6, -2, -2, 4), c(tuning_params[,i]), rats$time, X, Z, show_plot=FALSE)
  if (abs(zz$q - 25) < abs(acc - 25)){
    acc=zz$q
    tuners = tuning_params[,i]
  }
}
