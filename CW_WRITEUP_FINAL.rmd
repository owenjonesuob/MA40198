---
title: 'MA40198: Applied Statistical Inference Assessed Coursework 2018'
author: 'Candidates 20965, 20949 and 20969'
date: "13/12/2018"
output: pdf_document
geometry: [top=0.6in, left=0.6in, bottom=1in, right=0.6in]
header-includes:
  - \linespread{0.8}
---
...
```{r p1q5_pilot_run, fig.dim=c(7, 1), out.extra="", fig.cap="Samples from Metropolis-Hastings MCMC"}
iters <- 100000
burnin <- 2000
...
```{r p1q5_adj_run, fig.dim=c(7, 1), out.extra="", fig.cap="Samples from correlation-adjusted MCMC"}
...
```{r p1q5_adj_cor, echo=FALSE, fig.dim=c(7, 3), out.extra="", fig.cap="Correlation between parameters"}
...
```{r p1q5_ks, fig.dim=c(7, 1.5), out.extra="", fig.cap="Autocorrelation of parameters"}
...
The optimisation is of course dependent on our chosen value of the fatigue limit $\gamma$. By carrying out similar optimisations for different values of $\gamma$ across its potential range, we can see in Figure \ref{fig:p2q1_param_plots} that the parameter values change considerably. In particular, note the apparent negative correlation between `log_alpha` and `delta` (and the steadily tightening 95% confidence intervals for these parameters); and the rather more dramatic curve that `log_sigma` takes towards the upper end of the range of $\gamma$ (although the confidence interval seems to be of more or less constant width across the whole range).


```{r p2q1_param_plots, echo=FALSE, fig.dim=c(7, 2), out.extra="", fig.cap="Optimised parameters across the range of potential fatigue limits"}
...
```{r p2q4_pilot_run, fig.dim=c(7, 1), out.extra="", fig.cap="MCMC samples for the 5 fatigue parameters"}
...
```{r p2q4_adj_run, fig.dim=c(7, 1), out.extra="", fig.cap="Fatigue parameter values obtained with covariance-adjusted MCMC sampling"}
...
```{r p2q4_adj_cor, echo=FALSE, fig.dim=c(7, 3), out.extra="", fig.cap="Correlation between fatigue parameters"}
psych::pairs.panels(tail(adjusted$theta, 5000), pch = ".")
```

We now again check convergence to the stationary distribution using a Kolmogorov-Smirnov test.

```{r p2q4_ks_test, echo=FALSE, fig.dim=c(7, 1.5), out.extra="", fig.cap="Autocorrelation of fatigue parameters"}
...
```{r p2q5_mcmc, fig.dim=c(7, 1), out.extra="", fig.cap="MCMC sampling using full Bayesian posterior"}
...
$$\begin{aligned}
\log(\text{posterior}_\text{prop}) - \log(\text{posterior}_\text{old}) &= \log(f(n|\gamma_\text{prop})) + \log(\gamma_\text{prop}) + \log(f(n)) - \log(f(n|\gamma_\text{old})) - \log(\gamma_\text{old}) - \log(f(n))) \\
&= \log(f(n|\gamma_\text{prop})) + \log(\gamma_\text{prop}) - \log(f(n|\gamma_\text{old})) - \log(\gamma_\text{old})
\end{aligned}$$
