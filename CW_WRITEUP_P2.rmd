...
### Question 2.1: Maximum likelihood estimation of parameters
...
where
$$\epsilon_i \sim \text{Weibull} \left( \text{shape} = \frac{1}{\sigma}, \; \text{scale} = 1 \right)$$
...
$$N_i \sim \text{Weibull} \left( \text{shape} = \frac{1}{\sigma}, \; \text{scale} = \alpha (s_i - \gamma)^\delta \right)$$
...
This optimisation returns a set of optimised parameters, and note that we have asked `optim()` to return the Hessian matrix evaluated at these optimised values. In fact, since we have minimised the _negative_ log-likelihood, we have in fact been given the negative Hessian; inverting this yields an approximation to (asymptotic) covariance matrix, and therefore the standard errors for each parameter in $\mathbf{\theta}$ can be estimated by taking the square roots of the elements on the main diagonal. With these standard errors, we can calculate asymptotic 95% confidence intervals for each of the three parameters.
...
### Question 2.2: Estimation of fatigue limit
...
The code from Question 2.1 can be adapted easily by replacing `gamma` with `min_s/(1 + exp(-l_gamma))` and by adding `l_gamma` to the list of parameters for which we would like to return a gradient. Optimisation can then be performed as in Question 2.1.
...
### Question 2.3: Important quantiles of approximated distribution
...
### Question 2.4: Modelling fatigue limit as a random effect
