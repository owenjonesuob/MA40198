...
However, before attempting to produce any models it is always sensible to examine the data for any obvious patterns. We do not have space in this report to include a detailed exploratory analysis, but we can consider simple density plots of the uncensored observations, separated by whether or not treatment was received (see Figure \ref{fig:p1q1_density_plot}). Note that the distribution of the "treated" observations is possibly skewed slightly further to the left than the "untreated" distribution; but given the small sample size, this could just as easily be explained by natural variance in the data as it could by any potential effect of the treatment.

```{r p1q1_density_plot, fig.height=3, out.extra="", fig.cap="Density plots for }
library(ggplot2)
...
Hence the 95% asymptotic confidence interval for the treatment effect $\beta_1$ with random effects accounted for is `r conf_beta1_rand`. As you can see the confidence interval is much smaller when random effects are taken into account which implies that there are random effects present. The smaller confidence interval gives further evidence that the treatment is causing tumours to appear earlier in treated rats, i.e. it is carcenogenic. Whereas in questions 2 and 3 the top end of the confidence interval was very close to zero now it is lower and hence we can be fairly sure that, based on the observed data, this treatment is carcenogenic to rats. 

**Section 5**

We now use the same model as above, with random effects. We use a Bayesian estimation procedure to estimate unknown parameter values. First find the log of the posterior distribution to be used for MCMC. We use improper uniform priors for $\beta_0$, $\beta_1$ and $log(\sigma)$, and an exponential prior with rate 5 for $\sigma_b$. 
...
Create a general function for applying the MCMC algorithm.
```{r}
...
Run 100,000 iterations of MCMC using MLE results from part 4 as starting values, as we know these should already be reasonable parameter estimates. Tuning parameters are 0.1 for all parameters, and 0.03 for the random effects - these provide good acceptance probabilities. We have a burn-in period of 2000. 
...
```
Combine values for $\mathbf{\theta}$ and $\mathbf{b}$, and check for correlations between $\mathbf{\theta}$ using all values generated by MCMC excluding the burn-in period. 
```{r}
D <- cbind(pilot$theta, pilot$b)[-(1:burnin), ]
cor(D)[, 1:ncol(pilot$theta)][1:4,]
```
Results show that there is significant correlation between parameters, e.g. correlation of -0.71 between $\beta_0$ and $\beta_1$. We now adapt our MCMC approach by attempting to account for the correlations. At each step, the proposed $\mathbf{\theta}$ and $\mathbf{b}$ are generated using a multivariate normal proposal distribution, with covariance matrix cov(D) (where D is the matrix of combined MCMC results for $\mathbf{\theta}$ and $\mathbf{b}$, minus the burn-in period), centred around the previous values of the MCMC run. We use the last values generated by the above MCMC algorithm as starting values in the new algorithm. 
...
Running the algorithm, with 50,000 iterations and a burn-in period of 1000. The plots show good convergence to a stationary distribution due to them resembling white noise. 
...
Combining new values of $\mathbf{\theta}$ and $\mathbf{b}$, and checking their correlations and what their marginal densities look like. Marginal densities are shown on the diagonal, correlation plots between parameters to the left of the diagonal, and actual correlation values to the right. 
...
We now run a Kolmogorov-Smirnov test on each of the parameters to provide evidence for the fact that we have converged to the stationary distribution. We first calculate the ACFs, and from there calculate the autocorrelation lengths (ACLs). We then take a sample of the parameter, taking every ACLth value so the sample can be assumed to be independent. We then calculate two further subsamples of this sample, and run the K-S test on these. The p-values are all above 0.05, so at the 5% significance level there is no evidence to suggest that the two subsamples have come from different distributions. We can therefore assume we have converged to the stationary distribution. We have suppressed warnings for the ks function as the warnings provided were stating that p-values were approximated rather than calculated exactly due to a large sample size, we deemed this to not be a problem.
...
ks_pvals <- suppressWarnings({vapply(colnames(adjusted$theta), function(nm) {
  ...
  acl <- 2*sum(autocorr$acf) + 1 # Autocorrelation length
...
Calculate a 95% credible interval for $\beta_1$.
